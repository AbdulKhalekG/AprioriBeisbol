 ...
 * @param {Array[Object]} estads estadisticas totales
 * @returns Candidatos con el campo agregado de soporte
 */
const itemsetFrecuency = (candidates, estads) => {
  candidates.forEach((candidate) => {
    i = 0;
    estads.forEach((estad) => {
      let indexArray = candidate.itemset.map((item) => {
        return estad.items.indexOf(item);
      });
